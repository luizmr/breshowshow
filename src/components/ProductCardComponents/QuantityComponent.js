...
const QuantityComponent = ({
	...
	const handlePlus = () => {
		...
		} else if (quantityInNumber + 1 > obj.quantity) {
			handleDispatchAdd(product, obj.quantity);
		} else {
	...
	const handleConfirmProductQuantity = () => {
		const quantityInNumber = Number(product.quantity);
		if (quantityInNumber > obj.quantity) {
			if (Object.keys(product).length > 1) {
				handleDispatchAdd(product, obj.quantity);
			} else {
				handleDispatchAdd(obj, obj.quantity);
			}
	...
	const handleInputValueChange = (e) => {
		if (e.target.value <= obj.quantity) {
	...
	return (
		<div className="detail__quantity">
			...
			<div className="quantity__content">
				...
				<IconButton
					className={
						Number(product.quantity) < obj.quantity
							? 'plus__active'
							: ''
					}
					onClick={handlePlus}
